...
$(function () {
			...
			columnDefs: [
				...
				},
				{
					...
					render: function (data, type, full, meta) {
						...
						return (
							'<span style="padding: 6px 12px; border-radius: 6px; font-weight: 500; font-size: 0.9rem; display: inline-block; ' +
							style +
							'">' +
							$status +
							"</span>"
				...
				},
				{
					...
					render: function (data, type, full, meta) {
						return (
							...
							'<a href="#" class="btn btn-sm btn-icon view-member" data-id="' +
							full["user_id"] +
							'" title="View item details"><i class="bx bx-show"></i></a>' +
	...
	// 	$.ajax({
	// 		url: `${BASE_URL}Admin/Employee/index`, // Ensure the correct URL for form submission
	// 		type: "POST",
	...
	const creditScoreFormValidation = FormValidation.formValidation(creditScoreForm,
		{
			fields: {
				creditAmount: {
					validators: {
				...
				adjustmentNote: {
					validators: {
						notEmpty: {
							message: "Adjustment Note field is required",
						},
					},
				},
			},
	...
	const submitBtn = creditScoreForm.querySelector('button[type="submit"]');

	submitBtn.addEventListener("click", function (e) {
		e.preventDefault();
		creditScoreFormValidation.validate().then(function (status) {	
			if (status === "Valid") {
				showBlockUI();
				$.ajax({
					url: `${BASE_URL}Admin/Member/adjustCreditScore`, // Ensure the correct URL for form submission
					type: "POST",
					data: new FormData(creditScoreForm),
					contentType: false,
					cache: false,
					processData: false,
					dataType: "json",
					success: function (response) {
						...
						if (response.status) {
							Swal.fire({
								...
								window.location.href = BASE_URL + "Member/index";
