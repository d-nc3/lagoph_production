...
$(function () {
  ...
  var table = $('.datatables-cash-account'),
    select2 = $('.select2');
  ...
  if (table.length) {
    var dt = table.DataTable({
      ...
      serverSide: true,
      ajax: { 
        url: `${BASE_URL}Transaction_type_cash_account/dt_list`,
        type: "POST",
      },
      columns: [
        { data: null },
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            var $transaction_name = full['transaction_name'];
            return '<span>' + $transaction_name + '</span>';
          }
        },
        {
          targets: 3,
        ...
        },
        {
            targets: 4,
            render: function (data, type, full, meta) {
              var $code = full['code_of_cash_account'];
          ...
          render: function (data, type, full, meta) {
            return (
              '<div class="d-inline-block text-nowrap">' +
                '<a href="' + BASE_URL + 'Department/view/' + full['id'] + '" class="btn btn-sm btn-icon" title="View item details"><i class="bx bx-show"></i></a>' +
  ...
  var handleEditRows = function () {
    ...
    editButtons.forEach(d => {
        ...
        d.addEventListener('click', function (e) {
            ...
            showBlockUI();
            $.ajax({
              url: `${BASE_URL}Transaction_type_cash_account/get`, // Ensure the correct URL for form submission
              type: "POST",
              async: true,
              data: { id : id},
              dataType: 'json',
              success: function(response) {
                ...
                if(response) {
                  ...
                  $('#edit-transaction-type').val(response.transaction_type_id).trigger('change');
                  $('#edit-cash-account').val(response.cash_account_id).trigger('change');
  ...
  editForm.addEventListener('submit', function(event) {
    ...
    editFormValidation.validate().then(function(status) {
      if (status === 'Valid') {
        showBlockUI();
        $.ajax({
          url: `${BASE_URL}Transaction_type_cash_account/edit`, // Ensure the correct URL for form submission
          type: "POST",
  ...
  addform.addEventListener('submit', function(event) {
    ...
    addFormValidation.validate().then(function(status) {
      if (status === 'Valid') {
        showBlockUI();
        $.ajax({
          url: `${BASE_URL}Transaction_type_cash_account`, // Ensure the correct URL for form submission
          type: "POST",
