...
$(function () {
  ...
  var table = $('.datatables-transaction'),
    select2 = $('.select2');
  ...
  if (table.length) {
    var dt = table.DataTable({
      ...
      serverSide: true,
      ajax: { 
        url: `${BASE_URL}Transaction_types/dt_list`,
        type: "POST",
      },
      columns: [
        { data: null },
  ...
  var handleEditRows = function () {
    ...
    editButtons.forEach(d => {
        ...
        d.addEventListener('click', function (e) {
            ...
            showBlockUI();
            $.ajax({
              url: `${BASE_URL}Transaction_types/get`, // Ensure the correct URL for form submission
              type: "POST",
              async: true,
              data: { id : id},
              dataType: 'json',
              success: function(response) {
                ...
                if(response) {
                  ...
                  $('#edit-transaction-name').val(response.transaction_name);
  ...
  editForm.addEventListener('submit', function(event) {
    ...
    editFormValidation.validate().then(function(status) {
      if (status === 'Valid') {
        showBlockUI();
        $.ajax({
          url: `${BASE_URL}Transaction_types/edit`, // Ensure the correct URL for form submission
          type: "POST",
  ...
  addform.addEventListener('submit', function(event) {
    ...
    addFormValidation.validate().then(function(status) {
      if (status === 'Valid') {
        showBlockUI();
        $.ajax({
          url: `${BASE_URL}Transaction_types`, // Ensure the correct URL for form submission
          type: "POST",
