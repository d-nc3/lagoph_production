...
document.addEventListener("DOMContentLoaded", function () {
	(function () {
		...
		function fetchEvents(info, successCallback, failureCallback) {
			$.ajax({
				url: `${BASE_URL}Membership/get_events`,
				type: "GET",
				dataType: "json",
		 ...
		 FormValidation.formValidation(form, {
			fields: {
                event_date: {
                    validators: {
                        notEmpty: {
                            message: "Event Booking required",
                        }
                    }
                },            
			},
			...
			init: (instance) => {
				instance.on("plugins.message.placed", function (e) {
				  const wrapper = e.element.closest(".field-wrapper") || e.element.parentElement;
				  wrapper.appendChild(e.messageElement);
				});
			  }			  
		}).on("core.form.valid", function () {
			showBlockUI();
			$.ajax({
				url: `${BASE_URL}Membership/book_event`, // Ensure the correct URL for form submission
				type: "POST",
