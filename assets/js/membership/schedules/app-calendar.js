...
var select2Filters = $('.select2-filters');
...
document.addEventListener("DOMContentLoaded", function () {
	(function () {
		...
		function fetchEvents(info, successCallback, failureCallback) {
			$.ajax({
				url: `${BASE_URL}Membership/Membership/get_events`,
				type: "GET",
				dataType: "json",
				success: function (result) {
					...
					// Map the result from your database into FullCalendar-compatible format
					const events = result // adjust this as needed
						.map((event) => ({
							id: event.id,
							title: event.title,
					...
					// Send formatted events back to calendar
		...
		fv = FormValidation.formValidation(form, {
			fields: {
				...
				start_datetime: {
					validators: {
						...
						//     message: 'Start Date & Time is not valid'
						// },
						// callback: {
				...
				recurrence_rule: {
					validators: {
						regexp: {
							regexp: /^[A-Za-z0-9_;= ]+$/,
							message: "Recurrence rule format is invalid",
						},
					},
				},
				recurrence_end: {
					validators: {
						datetime: {
							format: "YYYY-MM-DD HH:mm",
							message: "Recurrence End Date is not valid",
						},
						// callback: {
						//     message: 'Recurrence End Date cannot be before the Start Date & Time',
						//     callback: function(value) {
						//         const startDate = $('#start_datetime').val();
						//         return new Date(value) > new Date(startDate);
						//     }
			...
			$.ajax({
				url: `${BASE_URL}Membership/Membership/add_event`, // Ensure the correct URL for form submission
				type: "POST",
				...
				success: function (response) {
					...
					if (response.status) {
						Swal.fire({
							...
							window.location.href = BASE_URL + `Event-scheduling` ;
						});
					} else {
