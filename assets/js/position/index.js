...
if (select2.length) {
  ...
  if (table.length) {
    var dt = table.DataTable({
      ...
      serverSide: true,
      ajax: { 
        url: `${BASE_URL}Admin/Position/dt_list`,
        type: "POST",
        data: function(d) {
          d.department_id = $("#filter-department-id").val();
          d.unit_id  = $("#filter-unit-id").val();
          ...
          render: function (data, type, full, meta) {
            return (
              '<div class="d-inline-block text-nowrap">' +
                '<a href="' + BASE_URL + 'Position/view/' + full['id'] + '" class="btn btn-sm btn-icon" title="View item details"><i class="bx bx-show"></i></a>' +
  ...
  var handleEditRows = function () {
    ...
    editButtons.forEach(d => {
        ...
        d.addEventListener('click', function (e) {
            ...
            showBlockUI();
            $.ajax({
              url: `${BASE_URL}Position/get`, // Ensure the correct URL for form submission
              type: "POST",
              async: true,
              data: { id : id},
              dataType: 'json',
              success: function(response) {
                ...
                if(response) {
                  ...
                  $('#edit-position-title').val(response.position_title);
  ...
  editForm.addEventListener('submit', function(event) {
    ...
    editFormValidation.validate().then(function(status) {
      if (status === 'Valid') {
        showBlockUI();
        $.ajax({
          url: `${BASE_URL}Admin/Position/edit`, // Ensure the correct URL for form submission
          type: "POST",
  ...
  addform.addEventListener('submit', function(event) {
    ...
    addFormValidation.validate().then(function(status) {
      if (status === 'Valid') {
        showBlockUI();
        $.ajax({
          url: `${BASE_URL}Admin/Position`, // Ensure the correct URL for form submission
          type: "POST",
  ...
  var handleDeleteRows = function () {
    ...
    deleteButtons.forEach(d => {
        ...
        d.addEventListener('click', function (e) {
            ...
            if (result.value) {
                $.ajax({
                    url: `${BASE_URL}Admin/Position/delete`,
                    type: "POST",
                    async: true,
                    data: { id : id},
                    dataType: 'json',
