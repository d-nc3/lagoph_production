...
class Transaction extends CI_Controller
{
    ...
    public function payment()
    {
        ...
        try {
            ...
            // Route payment process based on method
            switch ($info) {
                case NULL: // Direct Payment
    ...
    public function save_invoice_payment($info)
    {
        ...
        try {
            ...
            $this->_validate_payment($post_data); // Assume this throws on failure
            ...
            return $this->_send_json_response(
                TRUE,
    ...
    private function _process_payment($info, $post_data)
    {
        ...
        if ($transaction_category === '3') {
            ...
            if ($get_loan['remaining_balance'] == 0.00) {
                ...
                if (!$update_balance) {
                    log_message('An error occured in updating current credit. Please contact admin support');
                    throw new Exception('An error occured please contact admin support');
                }
            }
        }

        $official_receipt = $this->_create_official_receipt($post_data, $user_id, $billing_id, $cashier, $orNumber, $transaction_category);
    ...
    {
        $memberId = $this->M_member->get($user_id);
        ...
        $loan_repayment = $this->M_loan_repayment_schedule->pending_loan_repayment($memberId['id'], "payment_initiated");


        if (!$loan_repayment) {
            throw new Exception('No pending repayments for this loan.');
        }
        ...
        if ($balance === 0.0) {
            ...
            if (!$this->M_loans->update($loan['loan_id'], $update_data)) {
                log_message('error', 'Failed to update loan status to Fully Paid for ID: ' . $loan['loan_id']);
                $this->db->trans_rollback();
                throw new Exception('Error updating loan status.');
            }
        }
        // Start transaction
        $this->db->trans_start();
