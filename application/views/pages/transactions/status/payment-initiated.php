...
$date_issued = $payment_records['date_issued'] ?? '';
$invoice_number = $payment_records['invoice_number'] ?? '';
$particulars_id = $payment_records['invoice_particulars_id'] ?? '';
$invoice_id = $payment_records['cashiering_invoice_id'] ?? '';
$status = $payment_records['status'] ?? '';
$transaction_reference_id = $payment_records['transaction_reference_id'] ?? '';

$pending_cashiering_id = $invoice_particulars['cashiering_invoice_id'] ?? '';
$pending_particular_id = $invoice_particulars['id'] ?? '';

$particular = $invoice_particulars['item_id'] ?? '';
$unit_cost = $invoice_particulars['unit_cost'] ?? '';
$total_cost = $invoice_particulars['total_cost'] ?? '';

$current_date = date('Y-m-d');

$cashier_first_name = $_SESSION['first_name'] ?? '';
$cashier_last_name = $_SESSION['last_name'] ?? '';
?>
            ...
            <div class="card invoice-preview-card">
              <div class="card-body">
                <div class="row p-sm-3 p-0">
                  <div class="col-md-5">
                    <h3 class="fw-bold text-uppercase">LagoPH</h3>
                        ...
                        <div class="col-lg-8">
                          <dl class="row p-sm-2">
                            ...
                            <dd class="col-sm-6 text-md-end">
                              <div class="w-px-150">
                                <input type="date" class="form-control invoice-date" placeholder="YYYY-MM-DD"
                                  value=<?= $current_date ?> id="invoiceDate" />
                              </div>
                            </dd>
                          </dl>
                        </div>
                    ...
                    <table>
                      <tbody>
                        <tr class="mb-3">
                          ...
                          <td><input type="text" class="form-control mb-2" name="billing_address" id="billingAddress"
                              value=<?= $address ?>> </td>
                        </tr>
                        <tr class="mb-3">
                          <td class="pe-3">Email:</td>
                          <td><input type="text" class="form-control mb-2" name="billing_email" id="billingEmail"
                              value="<?= $email ?>"></td>
                        </tr>
                        <tr class="mb-3">
                          <td class="pe-3">Province:</td>
                          <td><input type="text" id="billingProvince" name="billing_province" class="form-control mb-2"
                              placeholder="Enter Province" value="<?= $province ?>"></td>
                        </tr>
                        <tr class="mb-3">
                          <td class="pe-3">Municipality:</td>
                          <td><input type="text" id="billingMunicipality" name="billing_municipality"
                              class="form-control mb-2" placeholder="Enter Municipality" value="<?= $municipality ?>">
                          </td>
                        </tr>
                      </tbody>
                    </table>
                ...
                <form class="form-repeater" id="formRepeater">
                    ...
                    <div data-repeater-item>
                      <div class="row">
                        ...
                        <div class="mb-3 col-lg-6 col-xl-2 col-12 mb-0">
                          <select class="form-select form-select2 items" id="itemDetails"
                            name="group-a[0][item_details]">
                        ...
                        <div class="mb-3 col-lg-6 col-xl-3 col-12 mb-0">
                          <input type="text" class="form-control item-price" id="itemPrice" name="group-a[0][item_cost]"
                            placeholder="PHP 1000" value="<?= $unit_cost ?>" />
                        </div>

                        <!-- Item Quantity -->
                        <div class="mb-3 col-lg-6 col-xl-2 col-12 mb-0">
                          <input class="form-control item-qty" name="group-a[0][item_quantity]" id="itemQuantity"
                            value="1" placeholder="1" min="1" max="50">
                        </div>

                        <!-- Total Cost -->
                        <div class="mb-3 col-lg-6 col-xl-3 col-12 mb-0">
                          <input type="text" class="form-control total-price" name="group-a[0][item_total]"
                            id="itemTotal" placeholder="1" readonly value="<?= $total_cost ?>" />
                        </div>
                  ...
                  <input type="hidden" id="invoiceId" name="invoice_id"
                    value="<?= $invoice_id ? $invoice_id : $pending_cashiering_id ?>">
                  <input type="hidden" id="particularsId" name="particulars_id"
                    value="<?= $particulars_id ? $particulars_id : $pending_particular_id ?>" />
                  <!-- Add Button -->
                  <div class="mb-0">
                    ...
                    <div class="col-md-6 mb-md-0 mb-3">
                      <div class="d-flex align-items-center mb-3">
                        ...
                        <input type="text" class="form-control" id="issuedBy" name="issued_by"
                          value="<?= $cashier_first_name ?> <?= $cashier_last_name ?>" />
                      </div>
                    </div>
                    <div class="col-md-6 d-flex justify-content-end">
                      <div class="invoice-calculations">
                        <div class="d-flex justify-content-between mb-2">
                          ...
                          <span class="fw-medium" id="subTotal">₱<?= number_format((float) $total_cost, 2) ?></span>
                          <!-- Ensure it's a float -->
                        </div>
                        <hr />
                        <div class="d-flex justify-content-between">
                          <span class="w-px-100">Total:</span>
                          <span class="fw-medium" id="total"
                            name="total_amount">₱<?= number_format((float) $total_cost, 2) ?></span>
                          <!-- Ensure it's a float -->
                        </div>
                ...
                <form class="payment-record" id="payment">
                  <?php if ($status !== NULL): ?>
                    <div class="mb-3 col-12">
                      <p class="mb-2">Transaction Category</p>
                      <select class="form-select mb-4 select2-search" id="transactionCategory"
                        name="transaction_category">
                        <option value=""></option>
                        <?php foreach ($transaction_category as $key => $val): ?>
                    ...
                    <input type="number" id="transactionId" name="transaction_reference_id"
                      value="<?= $transaction_reference_id ?>" hidden>
                    ...
                    <div class="mb-3 form-spacing">
                      ...
                      <input type="number" id="totalPayment" name="total_payment" class="form-control"
                        placeholder="₱0.00">
                    </div>
                    <div class="mb-3  form-spacing">
                      <p class="mb-2"> Payment Status</p>
                      <select class="form-select mb-4" id="paymentStatus" name="payment_status">
                        <option value="">Select</option>
                        <option value="Completed">Completed</option>
                        <option value="Rejected">Rejected</option>
                      </select>
                    </div>
                    <div class="mb-3 form-spacing" style="display: none;" id="commentContainer">
                      <label for="reference_no">Remarks</label>
                      <textarea id="comment" name="comment" class="form-control"
                        placeholder="Enter your comments"></textarea>
                    </div>
                    <div class="d-flex my-3" style="display: none;">
                      <a class="btn btn-label-secondary w-100 me-3" target="_blank"
                        href="<?= site_url('Official_receipt/receipt_view/' . $invoice_id) ?>">Preview</a>

                    </div>
                    <button type="button" class="btn btn-primary d-grid w-100 btn-save">
                      <span class="d-flex align-items-center justify-content-center text-nowrap"><i
                          class="bx bx-dollar bx-xs me-1 "></i>Issue Official Receipt</s>
                    </button>
                  </form>
                <?php endif; ?>
              </div>
